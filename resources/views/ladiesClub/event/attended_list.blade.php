...
@push('js')
    <script nonce="abc123">
        $(document).on('click', '.update-attendance', function () {
            ...
            if (confirm("Are you sure you want to toggle attendance for this member?")) {
                $.ajax({
                    url: "{{ route('app.eventMemberList.is_attend', ':id') }}".replace(':id', memberId),
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function (response) {
                        ...
                        //alert(response.message || 'Attendance updated!');
